...
if __name__ == "__main__":
    ...
    if args.mode == "run":
        args.gpu = True
        file_name = f'ae/energy_experiments/gelu_{device_name}_gpu.csv'
        gpu_kernel_launch_overhead = GeLU.gpu_kernel_launch_overhead()
    elif args.mode == "sim":
        args.simgpu = True
        file_name = f'ae/energy_experiments/gelu_{device_name}_sim.csv'
    else:
    ...
    for M in range(10, 30):
        ...
        if args.gpu:
            ...
            energy *= 1e9
            gelements = M / latency / 1e9
            power = (energy / 1e12) / latency
            
            print(f"{M}, {latency*1e3:.4f}ms, {gelements:.4f}GElements, {power:.2f}W, {energy}pJ, {grephics_freq:.2f}MHz", flush=True)
            with open(file_name, 'a') as f:
                f.write(f"{M}, {latency*1e3:.4f}ms, {gelements:.4f}GElements, {power:.2f}W, {energy}, {grephics_freq:.2f}\n")
        if args.simgpu:
            ...
            print(f"Performance: {M}, {latency*1e3:.4f}ms, {gelements}GElements, {power:.2f}W, {energy}")
            with open(file_name, "a") as f:
                f.write(f"{M}, {latency*1e3:.4f}ms, {gelements:.4f}GElements, {power:.2f}W, {energy['total']}, {energy['memory_to_l2_transfer']}, {energy['l2_to_l1_transfer']}, {energy['l1_to_l0_transfer']}, {energy['compute']}\n")
