...
if __name__ == "__main__":
    ...
    if args.mode == "run":
        args.gpu = True
        file_name = f'ae/energy_experiments/softmax_{device_name}_gpu.csv'
        gpu_kernel_launch_overhead = Softmax.gpu_kernel_launch_overhead()
    elif args.mode == "sim":
        args.simgpu = True
        file_name = f'ae/energy_experiments/softmax_{device_name}_sim.csv'
    else:
    ...
    titile = f"Performance of Softmax with M={M}"
    print(f"{titile}")
    ...
    N = 2**12
    titile = f"Performance of Softmax with N={N}"
    print(f"{titile}")
    for M in range(5, 16):
        M = 2**M
        model = Softmax(data_type=data_type_dict["fp16"])
