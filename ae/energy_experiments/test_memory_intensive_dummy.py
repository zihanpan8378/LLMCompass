from software_model.memory_intensive_dummy import MemoryIntensiveKernel
...
if __name__ == "__main__":
    ...
    if args.mode == "run":
        args.gpu = True
        file_name = f'ae/energy_experiments/memory_dummy_{device_name}_gpu.csv'
    elif args.mode == "sim":
        args.simgpu = True
        file_name = f'ae/energy_experiments/memory_dummy_{device_name}_sim.csv'
    else:
    ...
    titile = f"Performance of memory_dummy"
    print(f"{titile}")
    ...
    for M in range(5, 16):
        M = 2**M
        model = MemoryIntensiveKernel(data_type=data_type_dict["fp16"])
        _ = model(M)
        if args.gpu:
            # if test_overhead:
